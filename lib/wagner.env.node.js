...
wagner.extendResolve(function(name) {
	...
	try {
	  if(mod) {
			winston.debug('resolving cwd module: "' + name + '"')
	    resolved = mod.require(path.join(process.cwd(), name))

	    if(resolved) {
	      winston.debug('resolved cwd module: "' + name + '"')
	      return resolved
	    }
	  }
	}
	catch(ex) {
    winston.debug('failed to resolve cwd module: "' + name + '"')
    winston.debug(ex)	
	}

	try {
	  if(mod) {
			winston.debug('resolving command handler module: "' + name + '"')
	    resolved = mod.require(path.join(process.cwd(), 'commandHandlers', name))
	    ...
	    resolved = mod.require(path.join(process.cwd(), 'eventHandlers', name))
...
wagner.extendResolve(function(name) {
	...
	if(name === 'uuid') return this.require('node-uuid')
})
